/*** Class for Markov Chain Generation ***/
export class MarkovChain {
    ...
    createMarkovTable(arr) {
        ...
        for(let i = 0; i <= arr.length - this.order - 1; i++) {
            let gram = arr.slice(i, i + this.order).join('');
    ...
    getNextState() {
        ...
        let possibleNextStates = this.ngrams[this.currentState];
        
        let newValue = possibleNextStates[Math.floor(Math.random() * possibleNextStates.length)];
    ...
    generateSequence(startGram, length) {
        ...
        let generatedSequence = [currentGram];
